...
def get_page_divider(text):
    elem = div(_style="height:36px; margin-top: 10px; page-break-after: avoid;", _class="nobreak_div") \
           .append(div(_style="width: 100%; height: 12px; border-bottom: 1px solid #605E5C;")
                   .append(span(text, _style="font-size: 14px; background-color: #605E5C; "
...
def get_feature_importance_page(data):
    ...
    fi_left_heading = p("Understand factors that have impacted your model predictions the most. "
    ...
    heading_main = div(
        ...
        get_fi_image(data),
        _class="main")
    return div(get_page_divider("Feature relevance (explanability)"),
               fi_left_container,
               heading_main,
               _class="container")
...
def get_binary_cp_bar_plot(data, m):
    ...
    return get_bar_plot(y_data, x_data, legend=legend,
                        tickvals=tickvals, ticktext=ticktext, tickappend="%")
...
def get_de_bar_plot(data):
    ...
    return get_bar_plot(y_data, x_data, legend=legend, tickvals=tickvals, ticktext=ticktext, tickappend="%")
...
def get_bar_plot(y_data, x_data, legend=None,
                 threshold=None, tickvals=None, ticktext=None,
                 xrange=None, tickappend=""):
    fig = go.Figure()
    series = 0
...
def get_de_box_plot_image(data):
    ...
    for c in processed_label["data"]:
        c['label'] = c['label'] + "<br>" + str(int(100*round(c['population'], 3))) + "%"
...
def get_box_plot(data):
    fig = go.Figure()
    for i in data["data"]:
        fig.add_trace(go.Box(x=i["datapoints"], boxpoints=False,
...
def get_model_overview(data):
    ...
    if data["ModelType"] == "binary_classification":
        model_left_items.append(
            div(
                p("Classification: {} vs {}".format(data["classes"][0], data["classes"][1]))
            )
        )
    else:
        model_left_items.append(
            div(
                p("This is {} model".format(data["ModelType"]))
            )
        )
    ...
    model_left_items.append(
        div(
            ...
            p("This model is evaluated on a test set with {} datapoints.".format(len(data["y_test"])))
        )
    )
    ...
    model_main_items.extend(
        [h3("Target values"),
         p("Here are your defined target minimum/maximum performance levels "
    ...
    if data["runinfo"]:
        heading.append(
            p("Generated by {} on {}".format(data["runinfo"]["submittedBy"], data["runinfo"]["startTimeUtc"]))
        )
...
def get_cohorts_page(data, metric_config):
    left_elems = div(p("Observe evidence of model performance across your passed cohorts:"), _class="left")
    cp_heading_main = div(_class="main")
    heading_section = str(div(get_page_divider("Cohorts"), left_elems, cp_heading_main, _class="container"))
    ...
    def populate_cp_container(key, container, m, data):
        ...
        def get_regression_bar_plot(d, m):
            ...
            y_data = [[y['short_label'], str(int(y['population'] * 100)) + "%"] for y in d]
        ...
        message_lookup = {
            "cohorts": {"left_heading": "My Cohorts", "main_heading": "My prebuilt dataset cohorts"},
            "error_analysis_max": {"left_heading": "Highest ranked cohorts",
                                   "main_heading": "Highest ranked cohorts"},
            "error_analysis_min": {"left_heading": "Lowest ranked cohorts",
        ...
        container[key].append(
            get_cohorts_performance_container(filtered_data, m, get_regression_bar_plot, message_lookup[key])
        )

    cohort_performance_containers = {"cohorts": [], "error_analysis_max": [], "error_analysis_min": []}
...
def get_causal_page(data):
    left_elem = [div(p("Causal analysis answers real world what if "
    ...
    def get_table(data):
        horizontal_headings = ["Index", "Current<br>Treatment", "Recommended<br>Treatment", "Effect<br>Estimate"]
    ...
    for f in data["global_effect"].values():
        main_elems.append(
            div(
                ...
                get_causal_dot_plot(f["point"], f["ci_upper"] - f["point"], f["point"] - f["ci_lower"]),
                _class="nobreak_div"
            )
        )
        ...
        main_elems.append(
            h3(
                "Top data points responding the most to treatment on \"{}\":".format(f["feature"])
            )
        )
        main_elems.append(
            p(
        ...
        main_elems.append(
            get_table(
                [[p["index"], p["Current treatment"], p["Treatment"], round(p["Effect of treatment"], 2)]
...
class ClassificationComponents:
    ...
    def get_data_explorer_page(data):
        de_heading_left_elms = p("Evaluate your dataset to assess representation of identified subgroups:")
        ...
        def get_de_bar_plot(data):
            ...
            y_data = [str(c["label"]) + "<br>" + str(int(100 * c["population"])) + "%" for c in data]
            x_data = [100 * (list(c["prediction"]).count(class_0))/(len(c["prediction"])) for c in data]
            x_data = [[x, 100 - x] for x in x_data]
            ...
            png_base64 = get_bar_plot(y_data, x_data, legend=legend,
                                      tickvals=tickvals, ticktext=ticktext, tickappend="%")
            return div(img(_src="data:image/png;base64,{}".format(png_base64)), _class="image_div")
        ...
        for c in data:
            ...
            for i in c["data"]:
                de_main_elems.append(p(
                    "\"{}\" have {}% {}".format(
                        i["label"], round(i[c["primary_metric"]]*100, 1), c["primary_metric"]
                    )))
            de_main_elems.append(
                div(
                    p("Predicted classification output of the different subgroups are as follows:"),
        ...
        return str(
            div(get_page_divider("Data Explorer"), de_heading_left_container, de_main_container, _class="container"))
            ...
            return div(
                ...
                p(
                    ...
                    "= ({} + {}) / {}".format(
                        ...
                        data["confusion_matrix"]["tp"] + data["confusion_matrix"]["tn"] +
                        data["confusion_matrix"]["fp"] + data["confusion_matrix"]["fn"]
                    )
                )
            )
    ...
    def _get_confusion_matrix_grid(data):
        ...
        return table(
            tr(
                ...
                th(
                    "Acutal<br>\"{}\"".format(positive),
                    _class="header_cell"
                ),
                th(
                    "Acutal<br>\"{}\"".format(negative),
                    _class="header_cell"
                ),
            ),
            tr(
                th(
                    "Predicted<br>\"{}\"".format(positive),
                    _class="header_cell"
                ),
                td(
                    p(
    ...
    def get_model_performance_page(data):
        ...
        def get_metric_bar_plot(mname, data):
            y_0_filtermap = [True if i == data["classes"][0] else False for i in data["y_test"]]
            y_1_filtermap = [True if i == data["classes"][1] else False for i in data["y_test"]]
            class_0_metric = get_metric(mname, data["y_pred"][y_0_filtermap], data["y_test"][y_0_filtermap])
            class_1_metric = get_metric(mname, data["y_pred"][y_1_filtermap], data["y_test"][y_1_filtermap])

            y_data = ["Acutal \"{}\"".format(data["classes"][0]),
        ...
        for m in data["metrics"]:
            left_metric_elems.append(
                ClassificationComponents._get_model_performance_explanation_text(m, data)
            )
            main_elems.append(
                get_metric_bar_plot(m, data)
            )

        left_container = div(left_metric_elems, _class="left")
        main_container = div(main_elems, _class="main")
        return str(div(get_page_divider("Model Performance"), left_container, main_container, _class="container"))
    ...
    def get_fairlearn_page(data):
        left_elems = [div(p("Understand your model’s fairness issues "
        ...
        for f in data:
            ...
            for metric_key, metric_details in data[f]["metrics"].items():
                ...
                if metric_details["kind"] == "difference":
                    metric_section.append(p("Maximum difference in {} is {}".format(
                        metric_key,
                        round(metric_details["group_max"][1] - metric_details["group_min"][1], 2)))
                    )
                elif metric_details["kind"] == "ratio":
                    metric_section.append(p("Minimum ratio of {} is {}".format(
                        metric_key,
                        round(metric_details["group_min"][1]/metric_details["group_max"][1], 2)))
                    )
            left_elems.append(
                div(
                    metric_section,
        ...
        def get_fairness_bar_plot(data):
            y_data = [str(c) + "<br>" + str(int(100 * data[c]["population"])) + "%" for c in data]
            x_data = [100*(get_metric("selection_rate", data[c]["y_test"], data[c]["y_pred"])) for c in data]
            x_data = [[x, 100-x] for x in x_data]

            tickvals = [0, 25, 50, 75, 100]
            ticktext = [str(x) + "%" for x in tickvals]

            png_base64 = get_bar_plot(y_data, x_data,
                                      tickvals=tickvals, ticktext=ticktext, tickappend="%")
        ...
        def get_table(data):
            ...
            headings_td = [td(_class="header_cell")] + [td(x, _class="header_cell") for x in horizontal_headings]
            headings = thead(tr(headings_td, _class="row"), _class="table-head")
            ...
            for vh in vertical_headings:
                ...
                for m in metric_list:
                    row_data.append(
                        round(data["metrics"][m]["group_metric"][vh], 2)
                    )
                rows_elems.append(get_table_row(
                    vh,
                    row_data)
                )
        ...
        return str(div(get_page_divider("Fairness"), left_container, main_container, _class="container nobreak_div"))


class RegressionComponents:
    metric_name_lookup = {"mean_squared_error": "Mean squared error",
    ...
    def get_bar_plot_explanation_image():
        ...
        return div(img(_src="data:image/png;base64,{}".format(png_base64), _class="left_img"), _class="image_div")

    @staticmethod
    def get_data_explorer_page(data):
        de_heading_left_elms = p("Evaluate your dataset to assess representation of identified subgroups:")

        de_heading_left_elms.append(RegressionComponents.get_bar_plot_explanation_image())
            ...
            for i in c["data"]:
                de_main_elems.append(p(
                    "For {} datapoints, {} is the average of the {} between the actual value and the predicted value".format(
                        i["label"], round(i[c["primary_metric"]], 1), c["primary_metric"]
                    )))
            de_main_elems.append(
                div(
                    p("The distribution of prediction value for the different sets of data points are as follows:"),
    ...
    def get_metric_explanation_text(mname, mvalue):
        text_lookup = {"mean_squared_error": "{} is the average of the sqaured difference between the actual values and the predicted values.",
    ...
    def get_distributions_plot(data):
        bar_plot_data = {
            "data": [
                {
                    "label": "Predicted<br>Value",
                    "datapoints": data["y_pred"]
                },
                {
                    "label": "Ground<br>Truth",
                    "datapoints": data["y_test"]
                }
            ]
        }
    ...
    def get_model_performance_page(data):
        ...
        for m in data["metrics"]:
            left_metric_elems.append(
                h3(RegressionComponents.metric_name_lookup[m])
            )
            left_metric_elems.append(
                p(RegressionComponents.get_metric_explanation_text(m, data["metrics"][m]))
            )

        left_container = div(left_metric_elems, _class="left")

        main_distributions = div(h3("Distributions"),
                                 RegressionComponents.get_distributions_plot(data),
                                 _class="nobreak_div")
        main_histogram = div(h3("Histogram of your model errors "
    ...
    def get_fairlearn_page(data):
        ...
        for f in data:
            ...
            for metric_key, metric_details in data[f]["metrics"].items():
                left_elems.append(
                    p("\"{}\" has the highest {}: {}".format(metric_details["group_max"][0],
                                                             metric_key,
                                                             round(metric_details["group_max"][1], 2))
                      )
                )
                left_elems.append(
                    p("\"{}\" has the lowest {}: {}".format(metric_details["group_min"][0],
                                                            metric_key,
                                                            round(metric_details["group_min"][1], 2))
                      )
                )
        ...
        def get_table(data):
            ...
            horizontal_headings = ["Average<br>Prediction", "Average<br>Groundtruth"] + \
...
def to_pdf(html, output, wkhtmltopdf_path=None):
    ...
    wkhtmlconfig = None
    if wkhtmltopdf_path:
        wkhtmlconfig = pdfkit.configuration(
            wkhtmltopdf=wkhtmltopdf_path
        )

    pdfkit.from_string(html, output_path=output, options=options, configuration=wkhtmlconfig)
