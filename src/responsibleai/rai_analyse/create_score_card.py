...
from _rai_insight_data import RaiInsightData, PdfDataGen

from generate_pdf import RegressionComponents, ClassificationComponents, to_pdf, get_full_html
...
def parse_args():
    ...
    parser.add_argument("--rai_insights_dashboard", type=str, help="name:version", required=True)
    parser.add_argument("--pdf_output_path", type=str, help="pdf output path", required=True)
    parser.add_argument("--pdf_generation_config", type=str, help="pdf config", required=True)
    parser.add_argument("--predefined_cohorts_json", type=str, help="cohorts defintion", required=False)
    parser.add_argument("--local", type=bool, help="local run", required=False)
    parser.add_argument("--wkhtml2pdfpath", type=str, help="path to wkhtml2pdf", required=False)
...
def parse_threshold(threshold):
    ...
    if not match:
        _logger.warning("Unable to parse argument threshold {}. "
    ...
    try:
        ...
        _logger.warning("Unable to parse argument threshold {}. "
                        "Please refer to documentation for acceptable input.".format(threshold))
        _logger.warning("Exception message included which may be useful for your reference: {}".format(ex))
...
def main(args):
    ...
    if args.predefined_cohorts_json:
        with open(args.predefined_cohorts_json, 'r') as json_file:
            ...
            cohorts_map = {c["name"]: c["cohort_filter_list"] for c in cohorts_definition}
    ...
    if not args.local:
        add_properties_to_gather_run(dashboard_info, {"ScoreCardTitle": config["Model"]["ModelName"]})
...
class Workflow:
    ...
    def set_component_flags(self):
        self.cflags = {
            ...
            "cohorts": "Cohorts" in self.config or
                       len(self.raiinsight.list()["error_analysis"]["reports"]) > 0,
    ...
    def generate_pdf(self):
        ...
        to_pdf(
            get_full_html(body),
            os.path.join(self.args.pdf_output_path,
                         "scorecard.pdf"),
